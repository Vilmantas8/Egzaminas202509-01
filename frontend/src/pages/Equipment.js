...
function Equipment() {
  ...
  const loadEquipment = async () => {
    try {
      ...
      setFilteredEquipment(equipmentData); // Initialize filtered list
    } catch (error) {
  ...
  const loadReservations = async () => {
    try {
      ...
      // Ensure array remains array even on error
      setReservations([]);
    }
  };
  ...
  const isEquipmentAvailable = (equipmentId, startDate, endDate) => {
    ...
    if (!Array.isArray(reservations)) return true;
    
    const conflictingReservations = reservations.filter(reservation => {
      return (
        reservation.equipment && reservation.equipment._id === equipmentId &&
  ...
  const handleFiltersChange = (filters) => {
    // Ensure equipment is an array before filtering
    ...
    filtered.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        case 'name':
          ...
          break;
        case 'dailyRate':
          compareValue = (a.dailyRate || 0) - (b.dailyRate || 0);
          break;
        case 'category':
  ...
  const getAvailabilityStatus = (item) => {
    ...
    const upcomingReservations = reservations.filter(reservation => {
      const isForThisEquipment = reservation.equipment && reservation.equipment._id === item._id;
          ...
          <div className="col-md-8">
            <div className="d-flex align-items-center">
              <h5 className="mb-0 me-3">
                Rasta <span className="text-primary">{Array.isArray(filteredEquipment) ? filteredEquipment.length : 0}</span> įrangos vnt.
              </h5>
              {Array.isArray(filteredEquipment) && Array.isArray(equipment) && filteredEquipment.length !== equipment.length && (
                ...
                <div key={item._id} className="col-md-6 col-lg-4 mb-4">
                  <div className="card h-100 shadow-sm hover-shadow">
                    ...
                    <div className="card-body d-flex flex-column">
                      ...
                      {/* Equipment Name */}
                      <h5 className="card-title fw-bold">{item.name || 'Nepavadintas'}</h5>

                      {/* Equipment Description */}
                      <p className="card-text text-muted flex-grow-1">
                        {(item.description && item.description.length > 100) 
                          ? `${item.description.substring(0, 100)}...` 
                          : (item.description || 'Aprašymas nepateiktas')
                        }
                      </p>
                  ...
                  <div className="row text-center">
                    ...
                    <div className="col-sm-3 mb-3">
                      <div className="h4 text-info mb-1">
                        {Math.round(filteredEquipment.reduce((sum, item) => sum + (item.dailyRate || 0), 0) / filteredEquipment.length) || 0}€
                      </div>
