...
function Equipment() {
  ...
  const loadReservations = async () => {
    try {
      ...
      // Ensure reservations remains an array
      setReservations([]);
    }
  };
  ...
  const isEquipmentAvailable = (equipmentId, startDate, endDate) => {
    ...
    const conflictingReservations = reservationsArray.filter(reservation => {
      return (
  ...
  const handleFiltersChange = (filters) => {
    // Ensure equipment is always an array before filtering
    const equipmentArray = Array.isArray(equipment) ? equipment : [];
    let filtered = equipmentArray.filter(item => {
      // Basic filters
      const matchesSearch = 
  ...
  const getAvailabilityStatus = (item) => {
    ...
    const reservationsArray = Array.isArray(reservations) ? reservations : [];
    const upcomingReservations = reservationsArray.filter(reservation => {
